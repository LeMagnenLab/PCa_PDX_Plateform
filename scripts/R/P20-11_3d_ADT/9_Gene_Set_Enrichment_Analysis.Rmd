---
title: "8.P20-11 ADT (Only) treated (3 Days): Gene Set Enrichment Analysis"
...
Gene sets:
Wang NEPC -> PMID: 35789834 (Table S7 mmc8.xlsx)
Beltran NEPC -> 
Tang CRPC-> PMID: 35617398 (Zenodo, rnaSeq_figures.R)
...
```{r Load file: DE table ADT treatment groups}
...
ADT_DE_Genes = read.csv(file_path)

```

# Between ADT conditions
...
saveRDS(
  object = gse_obj_list,
  file = paste0(out_path,"/ADT_effect/", time_stamp(),"gse_results_list_per_collection_ADT.rds")
)

saveRDS(
  object = gse_obj_list_df,
  file = paste0(out_path,"/ADT_effect/" ,time_stamp(),"gse_results_list_per_collection_ADT.csv")
)
...
```{r GSEA dotplot: custom gene sets}
...
Restrict the analysis to the DE of main markers 

```{r Load all clusters DE file}
...
```{r Perform GSEA: compare cluster per collection}
...
DE_table_clusters_GSEA = DE_table_clusters_GSEA %>%
...
for(collection in seq_along(list_collection)){
  ...
  # Keep only GO terms with 10–300 genes (typical for BP)
...
```{r Export: GSEA compare cluster per collection}

out_path = create_exp_folder(
  organ = "Prostate" ,
  project = "Xenograft_Models",
  samples_ID = "P20-11_3d_ADT",
  exp = "9_Gene_Set_Enrichment_Analysis/clustering_effect/"
)

saveRDS(
  object = gse_obj_list,
  file = paste0(out_path, time_stamp(),"gse_results_list_compared_clusters_per_collection.rds")
)
...
saveRDS(
  object = gse_obj_list_df,
  file = paste0(out_path, time_stamp(),"gse_results_df_compared_clusters_per_collection.csv")
)

```

```{r Perform GSEA: compare cluster all collection at once}

term_sizes <- table(collection_df$term)
valid_terms <- names(term_sizes[term_sizes >= 10 & term_sizes <= 300])
collection_df_filtered <- collection_df[collection_df$term %in% valid_terms, ]

gse <- compareCluster(
  gene~cluster, 
  data = DE_table_clusters_GSEA,
  fun = 'enricher',
  TERM2GENE = collection_df_filtered,
  pvalueCutoff = 0.01,
  qvalueCutoff = 0.05,
  pAdjustMethod = "BH")
...
```{r Export GSE: compare cluster all collection at once}

saveRDS(
  object = gse,
  file = paste0(out_path, time_stamp(),"gse_results_compared_clusters_all_collection.rds")
)

write.csv(x =  gse_df,
  file = paste0(out_path, time_stamp(),"gse_results_compared_clusters_all_collection.csv")
)
...
file_path = get_exp_file_path(
  ...
  project = "Xenograft_Models",
  samples_ID = "P20-11_3d_ADT",
  prev_exp = "9_Gene_Set_Enrichment_Analysis/clustering_effect/",
  pattern = "gse_results_list_compared_clusters_per_collection.rds")
...
for (collection_id in seq_along(gse_obj_list)) {
  ...
  ggsave(plot = GOBP_dotplot,
         filename = paste0(out_path,time_stamp(), "plot_Dotplot_",collection_name,"_clusters.pdf"),
         width = 12,
         height = 8
  )
...
file_path = get_exp_file_path(
  ...
  samples_ID = "P20-11_3d_ADT",
  prev_exp = "9_Gene_Set_Enrichment_Analysis/clustering_effect/",
  pattern = "gse_results_compared_clusters_all_collection.rds")

gse = readRDS(file_path)

```

```{r Dotplot: compare cluster Dotplot (5top)}


dotplot = dotplot(gse, font.size = 6, showCategory = 5)

ggsave(plot = dotplot,
       filename = paste0(out_path,time_stamp(), "Dotplot_compare_clusters_all_collection.pdf"),
       width = 12,
       height = 8
)
