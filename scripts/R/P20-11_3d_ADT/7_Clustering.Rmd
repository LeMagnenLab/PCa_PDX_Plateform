---
title: "7. P20-11 ADT treated (3 Days): clustering"
...
fullsample_idents = get_exp_file_path(
  ...
  project = "Xenograft_Models",
  samples_ID = "P20-11_3d_ADT",
  prev_exp = "7_Clustering_Stability",  
...
parameters_to_test = list(
  c(pc = "20", k_param = 15, resolution = 0.5),
  c(pc = "20", k_param = 30, resolution = 0.5),
  c(pc = "20", k_param = 45, resolution = 0.5),
  c(pc = "20", k_param = 60, resolution = 0.5),
  c(pc = "20", k_param = 75, resolution = 0.5),
  c(pc = "20", k_param = 90, resolution = 0.5)
)
...
```{r Single UMAP colored with clustering of choice = louvain_pc20_k30_res0.5 -> (Figure version)}
...
plot <- ggplot() +
  ...
  geom_point(
    data = md,
    aes(x = UMAP_1, y = UMAP_2, fill = louvain_pc20_k30_res0.5), 
    alpha = 0.9, size = 3, 
    shape = 21,
    color = "gray10",
    stroke = 0.2) +
  ...
  geom_label(
    data = centroids,
    aes(x = UMAP_1, y = UMAP_2, fill = louvain_pc20_k30_res0.5, colour = "black", label = louvain_pc20_k30_res0.5),
    color = "black",  # Color for the cluster numbers
    size = 7,  # Adjust size as needed
    fontface = "bold"
  )
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_louvain_pc20_k30_res0.5_Figure.pdf"),
  device = "pdf",
  width = 6,
  height = 6,
)
...
plot <- ggplot() +
  # Rasterise the first layer
  geom_point(
    data = md %>% arrange(Multiplexing_Condition),
...
count_cluster_by_condition <- md %>% 
  group_by(Multiplexing_Condition, louvain_pc20_k30_res0.5) %>% 
  summarise(count = n())
...
pct_cluster_by_condition <- count_cluster_by_condition %>%
  ...
  group_by(Multiplexing_Condition) %>%
  arrange(louvain_pc20_k30_res0.5, Percentage)
...
count_cycle_by_cluster <- md %>% 
  group_by(louvain_pc20_k30_res0.5, cell_cycle_phase) %>% 
  summarise(count = n())
...
plot = ggplot(pct_cluster_by_condition, aes(x = 2, y = Percentage, fill = louvain_pc20_k30_res0.5)) +
  geom_bar(stat = "identity", width = 0.6, color = "black") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = pal_clustering) +
  xlim(0.5, 2.5) +  # crée le "trou"
...
plot = ggplot(pct_cycle_by_cluster, aes(x = louvain_pc20_k30_res0.5, y = Percentage, fill = cell_cycle_phase)) +
