...
```{r Violin plots for HALLMARK ANDROGEN RESPONSE}
...
for(gene_signature in gene_signature_scores){
  ...
  fml <- as.formula(paste(gene_signature, "~ louvain_pc20_k30_res0.5"))
  ...
  plot <- ggplot(md) +
    geom_violin(
      aes(x = louvain_pc20_k30_res0.5, y = .data[[gene_signature]], fill = louvain_pc20_k30_res0.5),
      alpha = 0.8,
      position = position_dodge(width = 0.8),
      scale = "width"
    ) +
...
md$louvain_pc20_k30_res0.5 = factor(md$louvain_pc20_k30_res0.5, levels = rev(c(4,5,3,1,2)))
...
plot <- ggplot(md) +
  geom_violin(
    aes(x = HALLMARK_ANDROGEN_RESPONSE_score, y = louvain_pc20_k30_res0.5, fill = louvain_pc20_k30_res0.5),
    alpha = 0.8,
    position = position_dodge(width = 0.8),
    scale = "width"
  ) +
...
```{r Z-score : per cluster on custom (litterature) NE gene sets }
...
for(signature_name in unique(custom_gene_sets_NE$term)){
  ...
  heatmap_anno = HeatmapAnnotation(
    Clusters = sce_obj$louvain_pc20_k30_res0.5,
  ...
  plot = Heatmap(name = "Z-score",
                 ...
                 column_split = sce_obj$louvain_pc20_k30_res0.5,  # Will seperate clusters
...
```{r Z-score : per cluster on HALLMARK ANDROGEN}

genes_on_Heatmap = HALLMARK %>% 
  filter(term == "HALLMARK_ANDROGEN_RESPONSE") %>%
  pull(gene)
...
pdf(file = paste0(out_path, time_stamp(), "plot_Heatmap_HALLMARK_ANDROGEN_clusters.pdf"),
    width = 12, height = 9.5)
...
draw(
  ...
  heatmap_legend_side = "bottom",
  annotation_legend_side = "left",
  column_title = "HALLMARK_ANDROGEN_RESPONSE",
  column_title_gp=grid::gpar(fontsize=16),
  merge_legend = F
)
...
```{r Heatmap of average scores}

md$louvain_pc20_k30_res0.5 = factor(md$louvain_pc20_k30_res0.5, levels = c(4,5,3,1,2))

Tang_score_mtx <- md %>%
  group_by(louvain_pc20_k30_res0.5) %>%
  summarise(across(contains("Tang"), mean)) %>%
  tibble::column_to_rownames("louvain_pc20_k30_res0.5")  # Makes the walktrap_75 cluster IDs row names
...
plot_list = list()
gene_list = c("AR","ALDH1A1", "KLK3","PROX1", "KLK2", "CHGB")
...
for(gene in seq_along(gene_list)){
  ...
  plot <- ggplot(data = md, aes(x = UMAP_1, y = UMAP_2)) +
    # Rasterize points for better performance in large datasets
    ggrastr::rasterise(
      geom_point(
    ...
    # Apply a custom continuous color scale using viridis::magma
...
ggsave(
  plot = list_of_grobs,
  filename = paste0(out_path, time_stamp(), "plots.pdf"),
