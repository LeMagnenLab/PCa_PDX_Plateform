---
title: "7. P20-11 ADT+ENZ treated (3 Days): clustering"
...
```{r Single UMAP colored with clustering of choice (walktrap_15)}

# Calculate centroids for each cluster
centroids <- md %>%
  group_by(walktrap_15) %>%
  summarise(
    UMAP_1 = mean(UMAP_1),
    UMAP_2 = mean(UMAP_2),
    .groups = 'drop'
...
plot <- ggplot() +
  ...
  ggrastr::rasterise(
    geom_point(
      data = md,
      aes(x = UMAP_1, y = UMAP_2, fill = walktrap_15), 
      alpha = 0.7, size = 1.3, 
      shape = 21,
      color = "gray10",
  ...
  geom_text(
    data = centroids,
    aes(x = UMAP_1, y = UMAP_2, label = walktrap_15),
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_Single_walktrap_15.pdf"),
  device = "pdf",
  width = 10,
  height = 8,
)
...
```{r Single UMAP colored with clustering of choice (walktrap_15) (Figure version)}
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_Single_walktrap_15_Figure.pdf"),
  device = "pdf",
  width = 6,
  height = 6,
)
...
stat <- metadata_df %>% 
  group_by(Multiplexing_Condition, walktrap_15) %>% 
  summarise(count = n()) 
...
plot = ggplot(data, aes(x = Percentage, y = walktrap_15, fill = Multiplexing_Condition)) +
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_Proportion_Treatment_vs_Clustering_walktrap_15", ".pdf"),
  device = "pdf",
  width = 8,
  height = 4
)
