---
title: "6. PDOX and PDOXO (P20-11 & P20-23): dimension reduction"
...
out_path = create_exp_folder(
  ...
  exp = "6.0_Dimension_Reduction_No_Regression"
)
...
file_path = get_exp_file_path(
  ...
  project = "Xenograft_Models",
  samples_ID = "All_PDOXs_PDOXOs",
  prev_exp = "4_Normalization_Rescaling", 
...
# On integrated sce (without batch correction)
...
combined.dec <- combineVar(unlist(list_dec))
...
sce_obj <- runUMAP(
  sce_obj,
  dimred = "PCA.1-20",
  name = "UMAP_on_PCA.1-20_No_BatchCorrection",
  n_neighbors = 300,
  min_dist = 0.7)
...
# get embeddings from 2 first PC
coords <- as_tibble(reducedDim(sce_obj, "UMAP_on_PCA.1-20_No_BatchCorrection"))[,1:2]
...
## UMAP colored with different parameters 

```{r UMAP facetted: colored per library}
...
for (sample in unique(md$SampleGroup)) {
  ...
  plot <- ggplot() +
    ...
    ggtitle(label = sample,
            subtitle = "No batch correction applied" ) +
    theme_classic() +
...
```{r Single UMAP: colored with patient ID}
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_PAtient_ID.pdf"),
  device = "pdf",
  width = 10,
  height = 8,
)
...
```{r Single UMAP: colored with clean ID}
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_Line_ID.pdf"),
  device = "pdf",
  width = 10,
  height = 8,
)
...
```{r Single UMAP: colored with model system}
...
plot <- ggplot() +
  ...
  ggrastr::rasterise(
    geom_point(
      data = md,
      aes(x = UMAP_1, y = UMAP_2, fill = Model_System), 
      alpha = 0.9, size = 1.3, 
      shape = 21,
      color = "gray10",
      stroke = 0.2),  # Lighter stroke color (adjust as needed)),# Ensure shape supports fill and color
...
ggsave(
  plot = plot,
  filename = paste0(out_path,time_stamp(),"plot_UMAP_Model_System.pdf"),
  device = "pdf",
  width = 10,
  height = 8,
)
